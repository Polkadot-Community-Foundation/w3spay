 ...
 * `message` listener attaches before the worker can post `"ready"`. With the
 * WASM module cached from a prior mount the worker can post `"ready"` during
 * camera-startup awaits; a listener attached later never sees it, the loop
 * hangs on `await bridge.ready`, and the viewfinder freezes. See
  ...
  const scanFrame = (now: DOMHighResTimeStamp, _metadata: VideoFrameCallbackMetadata) => {
    ...
    void (async () => {
      try {
        ...
        decodeAttempts += 1;
        if (decodedText != null) {
          // Dedup by payload so the scanner keeps reading after a bad/unsupported
          // QR — handleDecoded does not always navigate away (an unsupported QR
          // sets lastBadScan and stays on the scan screen). Without this the loop
          // freezes on the first decode.
...
export async function startZxingWasmScanner(
  ...
  const myTurn = startupQueue.catch(() => undefined).then(async () => {
    ...
    // module cached from a prior mount, the worker can post `"ready"` within
    // the same task as `video.play()`'s await yield, and a listener attached
    // after would miss it and hang the decode loop on `bridge.ready`. This is
    // the "scan again doesn't detect" regression.
