 ...
 * with step counter, headline, balance chip, and a wallet-overlay button.
 * The active backend owns the `<video>` it mounts in the camera region.
 */
...
export interface ScanningScreenProps {
  ...
  /**
   * `true` once the balance permission modal resolved. The host renders one
   * permission modal at a time — starting the camera request before the
   * balance one closes silently drops it.
   */
