...
/**
 * Coinage-backed customer-side payment through w3spay's narrow `PaymentHost`
 * interface. The host owns the vault and runs the on-chain extrinsics;
 * `sendPayment` resolves the destination, hands the host the amount and bytes,
 * and returns the receipt id for the done screen.
 */
