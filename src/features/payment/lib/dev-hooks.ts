...
/**
 * Dev-standalone escape hatch. `vite dev` runs on plain HTTP, so on
 * non-localhost origins `getUserMedia` is blocked and the camera screen never
 * advances. `useDevHooks` wires console-callable helpers to drive the
 * scan → confirm → done flow without a real camera:
 *
