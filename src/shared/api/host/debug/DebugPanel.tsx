 ...
 *
 * For mobile-host debugging: the host's webview ships only the built SPA with
 * no DevTools, so this can ride along in production (gated by an env flag) to
 * diagnose a "stuck on boot splash" issue on iOS without redeploying.
 *
 * Tabs: Console (`console.*` / `window.onerror` / `unhandledrejection`
 * capture), Timeline (boot-event markers from `wallet.ts`), Host (live
 * `useHostWalletSnapshot()`), Actions (retry / reset / clear / copy).
 *
 * Each tab can copy or open a dump modal — the modal is the iOS fallback for
 * when `navigator.clipboard.writeText` is blocked outside a user gesture.
 *
 * `getSnapshot` MUST be referentially stable between store updates, or
 * `useSyncExternalStore` re-renders the panel in a tight loop (the store
 * caches its snapshot). The capture (`installConsoleCapture`) is module-level
 * and idempotent, staying installed for the page lifetime once the panel
 * mounts.
 */
...
// Resize constraints. Min keeps the panel readable on a phone; max is the
// viewport minus a 16px margin on each side so it can go near-fullscreen.
const MIN_WIDTH = 240;
...
function loadGeometry(): PersistedGeometry | null {
  ...
  try {
    ...
    // Ignore — localStorage may throw in sandboxed contexts (Safari private mode).
  }
  return null;
}
...
/**
 * Clamp a candidate (width, height) to the panel's min/max bounds — during
 * resize and at mount (to re-fit a geometry persisted on a different screen).
 */
...
/**
 * Clamp a (right, bottom) anchor so the panel's left/top edges stay inside the
 * viewport after a resize (neither edge is allowed below 0).
 */
...
/** `useSyncExternalStore`-friendly wrapper; `getSnapshot` is referentially
 *  stable so this re-renders only when the store actually changes. */
...
/** Try to write `text` to the clipboard. iOS Safari blocks writes outside a
 *  user gesture, so returns whether it succeeded — callers fall back to a
 *  textarea modal on false. */
...
function DumpModal({
  ...
  // Auto-select on mount so the user can long-press → copy on iOS. `setTimeout`
  // waits for the modal to paint, else focus is lost when scrolled into view.
  useEffect(() => {
    const id = setTimeout(() => {
        ...
        >
          ...
          <button
            ...
            onClick={async () => {
              ...
              if (!ok) {
                // Clipboard API blocked (iOS, no user gesture); the textarea
                // is already selected so the user can long-press → copy.
...
export interface DebugPanelProps {
  /** If `true`, the panel is initially open. Default `false`. */
  defaultOpen?: boolean;
  /** Initial filter for the console tab. */
...
export function DebugPanel(props: DebugPanelProps = {}): React.ReactElement {
  ...
  /**
   * Saved on maximize so the next tap restores the pre-maximize geometry.
   * `null` means no restore geometry — maximize just fills the viewport.
   */
  ...
  // Hydrate position + size from localStorage on first mount; subsequent moves
  // are persisted via the effect below.
  useEffect(() => {
  ...
  // Persist position + size on change. A ref skips the mount-time write so the
  // initial render doesn't clobber a concurrent update from another tab.
  ...
  // Stream the host-wallet snapshot into the store so the Host tab can render
  // it without owning wallet init. Destructure the fields we care about so the
  // effect re-runs only when one of THOSE changes, not on every wallet render.
  ...
  // Wallet store no longer tracks allowances; field kept for schema compat but
  // always 0. Apps driving `claimResourceAllowances` can wire their own.
  ...
  const onResizePointerDown = useCallback(
    (direction: ResizeDirection) => (event: React.PointerEvent) => {
      ...
      try {
        ...
        // Older Safari rejects setPointerCapture on some elements. Best-effort
        // — the global pointermove listener below still tracks the resize.
      }
      resizeRef.current = {
        direction,
  ...
  // Resize on pointermove. Listen on `window` (not the handle) so a finger that
  // drags off the small corner handle on mobile still tracks the resize.
  useEffect(() => {
    const onMove = (event: PointerEvent) => {
      ...
      // Each direction contributes independently in viewport coords:
      // right/bottom are offsets from the edges; dragging an edge away from
      // origin grows the panel and shifts the offset inversely.
      ...
      if (r.direction.includes("w")) {
        // Left edge moves, right edge stays put — width changes inversely with dx.
        newWidth = r.startWidth - dx;
      }
      if (r.direction.includes("n")) {
        // Top edge moves, bottom edge stays put — height shrinks as you drag down.
        newHeight = r.startHeight - dy;
      }
  ...
  const handleMaximizeToggle = useCallback(() => {
    if (isMaximized) {
      // Restore the pre-maximize geometry; the persistence effect saves it next.
      setPosition(
        clampedPosition(
      ...
      // Save the current geometry, then fill the viewport (with a margin) —
      // `clampedSize` picks the size from the current viewport.
  ...
  return (
    <>
      ...
      >
        <div
          ...
          style={{
            ...
            userSelect: "none",
            touchAction: "none",
            // Corner handles overlap the header corners; they sit at z-index 1
            // to win clicks there. The bulk of the header is still draggable.
            position: "relative",
          }}
        >
      ...
      {/* Resize handles — 4 corners + 4 edges. Sibling divs at z-index 1 so
          they sit above the panel content and receive pointer events first.
          `touchAction: "none"` is critical on mobile; hit area is large
          (20px corners, 8px edges) for touch. */}
...
const RESIZE_HANDLE_STYLES: Record<ResizeDirection, React.CSSProperties> = {
  // Edges — 8px strips, invisible at rest (cursor + touchAction carry
  // discoverability); corners are the primary affordance.
  ...
  // Corners — 20x20 (mobile thumb target) with a 1px dot for the affordance.
...
const RESIZE_HANDLE_INNER: Record<ResizeDirection, React.CSSProperties> = {
  // Only the corner handles get a visible dot; edges are pure hit targets.
  nw: { top: 1, left: 1 },
  ne: { top: 1, right: 1 },
  sw: { bottom: 1, left: 1 },
  se: { bottom: 1, right: 1 },
...
function ResizeHandle({
  ...
  return (
    <div
      ...
      // `touchAction: "none"` is the critical mobile detail — without it a
      // drag from a corner handle scrolls the page instead of resizing.
      style={{
        position: "absolute",
        zIndex: 1,
        touchAction: "none",
