 ...
 * host / balance / merchant queries so they fetch at boot, and binds the
 * dev-standalone `window.__w3spayDev*` console hooks (tree-shaken in prod).
 *
 * Permission strategy: data-access and payment permissions are requested at
 * boot, but camera permission is requested on the scan page only — so the iOS
 * native camera sheet can't race the payment-permission modal. The camera
 * probe fires inside `<ScanPage>` gated on `balancePermissionResolved`.
 */
